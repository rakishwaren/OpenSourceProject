  ...
  xmlhttp.onreadystatechange = function() {
    if (this.readyState == this.DONE && this.status == 200) {
      ...
      if ('results' in data) {
        ...
        for (var i in result) {
          texts += 'Definition: ' + result[i].definition + '<br />';
          texts += 'Part of Speech: ' + result[i].partOfSpeech + '<br />';

          if ('synonyms' in result[i]) {
            texts += 'Synonyms: ';

            for (var j in result[i].synonyms) {
              if (j == result[i].synonyms.length - 1) {
                texts += result[i].synonyms[j] + '<br />';
              } else {
                texts += result[i].synonyms[j] + ', ';
              }
            }
          }

          if ('typeOf' in result[i]) {
            texts += 'Type Of: ';

            for (var j in result[i].typeOf) {
              if (j == result[i].typeOf.length - 1) {
                texts += result[i].typeOf[j] + '<br />';
              } else {
                texts += result[i].typeOf[j] + ', ';
              }
            }
          }
